...
const DEFAULT_INK_GRADIENT_START = "#6d5bd0";
const DEFAULT_INK_GRADIENT_END = "#2e92a6";
const DEFAULT_BACKGROUND_GRADIENT_START = "#fff6d6";
const DEFAULT_BACKGROUND_GRADIENT_END = "#eeeaff";
...
let linePointDragState = null;
let editorSearchQuery = "";
let insightPanelOpen = true;
let inkColor = DEFAULT_INK_COLOR;
let inkOpacity = DEFAULT_INK_OPACITY;
let inkGradientEnabled = false;
let inkGradientStart = DEFAULT_INK_GRADIENT_START;
let inkGradientEnd = DEFAULT_INK_GRADIENT_END;
...
function normalizeData(source) {
  ...
  source.boards.forEach(board => {
    ...
    board.pages.forEach(page => {
      ...
      page.backgroundGradientEnabled ??= page.backgroundGradient !== "none";
      page.backgroundGradientStart ||= DEFAULT_BACKGROUND_GRADIENT_START;
      page.backgroundGradientEnd ||= DEFAULT_BACKGROUND_GRADIENT_END;
...
function applyCanvasAppearance(page = getPage()) {
  ...
  const gradient = page.backgroundGradientEnabled !== false
    ? `linear-gradient(135deg, ${page.backgroundGradientStart || DEFAULT_BACKGROUND_GRADIENT_START}, ${page.backgroundGradientEnd || DEFAULT_BACKGROUND_GRADIENT_END})`
    : CANVAS_GRADIENTS.none;
  canvas.style.setProperty("--canvas-gradient", gradient);
}
...
function lineControlMarkup(drawing) {
  ...
  return drawing.points.map((point, index) => `<circle class="line-control-point" data-line-point data-drawing-id="${drawing.id}" data-point-index="${index}" cx="${point[0].toFixed(1)}" cy="${point[1].toFixed(1)}" r="${index === 0 || index === drawing.points.length - 1 ? 5 : 6}"/>`).join("");
}
...
function drawConnections() {
  ...
  const doodles = (page.drawings || []).map(drawing => {
    ...
    const gradient = drawing.gradient === "custom"
      ? [drawing.gradientStart || DEFAULT_INK_GRADIENT_START, drawing.gradientEnd || DEFAULT_INK_GRADIENT_END]
      : INK_GRADIENTS[drawing.gradient || "none"];
...
function updateContextOptions() {
  ...
  if (page) {
    ...
    const canvasGradientToggle = $("#canvasGradientToggle");
    const canvasGradientStart = $("#canvasGradientStartInput");
    const canvasGradientEnd = $("#canvasGradientEndInput");
    if (canvasColor) canvasColor.value = page.backgroundColor || "#faf9f4";
    if (canvasGradientToggle) canvasGradientToggle.checked = page.backgroundGradientEnabled !== false;
    if (canvasGradientStart) canvasGradientStart.value = page.backgroundGradientStart || DEFAULT_BACKGROUND_GRADIENT_START;
    if (canvasGradientEnd) canvasGradientEnd.value = page.backgroundGradientEnd || DEFAULT_BACKGROUND_GRADIENT_END;
    const backgroundGroup = $('[data-context-group="background"]');
    if (backgroundGroup) backgroundGroup.classList.toggle("gradient-off", page.backgroundGradientEnabled === false);
  }
  ...
  if ($("#inkGradientToggle")) $("#inkGradientToggle").checked = inkGradientEnabled;
  if ($("#inkGradientStartInput")) $("#inkGradientStartInput").value = inkGradientStart;
  if ($("#inkGradientEndInput")) $("#inkGradientEndInput").value = inkGradientEnd;
  const inkGroup = $('[data-context-group="ink"]');
  if (inkGroup) inkGroup.classList.toggle("gradient-off", !inkGradientEnabled);
...
function handlePointerMove(event) {
  if (linePointDragState) {
    const point = drawingPoint(event);
    const { drawing, pointIndex } = linePointDragState;
    if (!linePointDragState.historyPushed) {
      pushHistory();
      linePointDragState.historyPushed = true;
    }
    const isEndpoint = pointIndex === 0 || pointIndex === drawing.points.length - 1;
    drawing.points[pointIndex] = isEndpoint ? snapToLineEndpoint(point, drawing.id) : point;
    drawConnections();
    return;
  }
...
function handleDrawingPointerDown(event) {
  const controlPoint = event.target.closest("[data-line-point]");
  if (controlPoint && currentTool === "select") {
    const drawing = (getPage().drawings || []).find(item => item.id === controlPoint.dataset.drawingId);
    if (!drawing) return;
    event.preventDefault();
    event.stopPropagation();
    ...
    linePointDragState = {
      drawing,
      pointIndex: Number(controlPoint.dataset.pointIndex),
      historyPushed: false
    };
    controlPoint.setPointerCapture(event.pointerId);
    return;
  }
...
function addLinePoint(event) {
  if (event.target.closest("[data-line-point]")) return;
  const path = event.target.closest("[data-drawing-id]");
  if (!path) return;
  const page = getPage();
...
function snapToLineEndpoint(point, excludedDrawingId = null, radius = 16) {
  const endpoints = (getPage()?.drawings || [])
    .filter(drawing => drawing.kind === "line" && drawing.id !== excludedDrawingId && drawing.points?.length > 1)
    .flatMap(drawing => [drawing.points[0], drawing.points[drawing.points.length - 1]]);
  let snapped = point;
...
function startDrawing(event) {
  ...
  event.preventDefault();
  pushHistory();
  const point = currentTool === "connector" ? snapToLineEndpoint(drawingPoint(event)) : drawingPoint(event);
  const drawing = currentTool === "connector"
    ? { id: uid("line"), kind: "line", color: "#2e92a6", opacity: 1, size: 4, gradient: "none", points: [point, point] }
    : { id: uid("doodle"), kind: "ink", color: inkColor, opacity: inkOpacity, size: penSize, gradient: inkGradientEnabled ? "custom" : "none", gradientStart: inkGradientStart, gradientEnd: inkGradientEnd, points: [point] };
  getPage().drawings ||= [];
...
function continueDrawing(event) {
  ...
  if (activeDrawing.kind === "line") {
    ...
    const angledPoint = event.shiftKey ? snapOrthogonalPoint(start, point) : point;
    activeDrawing.points[activeDrawing.points.length - 1] = snapToLineEndpoint(angledPoint, activeDrawing.id);
    drawConnections();
    return;
  }
...
function bindEvents() {
  ...
  $("#canvasGradientToggle").addEventListener("change", event => updatePageBackground({ backgroundGradientEnabled: event.target.checked }));
  $("#canvasGradientStartInput").addEventListener("input", event => updatePageBackground({ backgroundGradientStart: event.target.value }));
  $("#canvasGradientEndInput").addEventListener("input", event => updatePageBackground({ backgroundGradientEnd: event.target.value }));
  ...
  $("#inkGradientToggle").addEventListener("change", event => {
    inkGradientEnabled = event.target.checked;
    updateContextOptions();
  });
  $("#inkGradientStartInput").addEventListener("input", event => {
    inkGradientStart = event.target.value;
    updateContextOptions();
  });
  $("#inkGradientEndInput").addEventListener("input", event => {
    inkGradientEnd = event.target.value;
    updateContextOptions();
  });
